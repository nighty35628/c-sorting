...
### 3.2 具名地理分类策略
通过集成腾讯地图 WebService API，应用实现了“反向地理编码”功能。针对高频触发 API 可能带来的延迟与频率限制难题，我构建了以下技术保障：
1. **局部内存缓存**：对相近坐标的查询结果进行持久化缓存，极大减少了网络开销。
2. **频率控制 (Rate Limiting)**：强制执行 `300ms` 的请求间隔，确保符合第三方服务的并发规范。
3. **分层异常处理**：在网络抖动或 API key 受限时，程序会自动回退至日期分类模式，确保处理流程不中断。

---

## 4. 并发管理：基于 QThread 的非阻塞异步设计

照片分类涉及大量的文件 I/O 和网络请求，这些操作若在主线程执行会导致 UI 进入“假死”状态。在 C-SORTING 中，我通过继承 `QThread` 实现了业务逻辑与界面交互的高度解耦。
...
> - **Framework**: PyQt6
> - **Key Libraries**: Pillow (Image Processing), Requests (API Interaction)
